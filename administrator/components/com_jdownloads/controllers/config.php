...
class jdownloadsControllerconfig extends jdownloadsController
{
	...
	public function save() {
        ...
        // specail handling for the multiple select box
        $dummy = $jinput->get('sortorder_fields',  array(), 'array');
        $config_data['sortorder.fields'] = implode(',', $dummy);        
        ...
        $config_data['send.mailto']        = JDownloadsHelper::cleanEMailAddresses($config_data['send.mailto']);
        $config_data['send.mailto.upload'] = JDownloadsHelper::cleanEMailAddresses($config_data['send.mailto.upload']);
        $config_data['send.mailto.report'] = JDownloadsHelper::cleanEMailAddresses($config_data['send.mailto.report']);
        ...
        if (!$config_data['use.alphauserpoints']){
            ...
            $config_data['user.can.download.file.when.zero.points'] = '0';
        }   		
