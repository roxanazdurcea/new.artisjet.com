...
$bloggers = $model->getBloggers($sort, $limit, $filter);
$config = EB::config();
...
if (!empty($bloggers)) {
    foreach ($bloggers as $row) {
        ...
        $biographyTotal = JString::strlen(strip_tags($blogger->getBiography()));
        $bioLength = $params->get('bio_length', 50);
        $biography = $blogger->getBiography();

        if ($biographyTotal > $bioLength) {
            $biography = JString::substr($biographyTotal, 0, $bioLength) . '...';
        } 
